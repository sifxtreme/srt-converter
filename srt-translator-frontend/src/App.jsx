...
const SubtitleTranslator = () => {
  ...
  const handleTranslate = async (setId) => {
    ...
    const eventSource = new EventSource(`http://localhost:3001/translation-progress/${setId}`);
    ...
    // Start translation
    try {
      const response = await fetch(`http://localhost:3001/translate/${setId}`, {
        method: 'POST',
      });
  ...
  const handleDownload = async () => {
    ...
    try {
      window.location.href = `http://localhost:3001/download/${currentSetId}`;
    } catch (err) {
